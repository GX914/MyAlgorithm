...
/**
 * 冒泡排序（时间复杂度为n的平方，最好情况为n，空间复杂度为1，稳定）
 *
 * 从第一个和第二个比较大小，然后调换位置，再比较第二个和第三个，调换位置，依次类推到最后一位。
 * 外层用i计数，每排序一轮，所有元素最大的就会调换到最后一位，所有每一轮排序的元素数为length-1.
 *
 * 冒泡排序可以理解为，每一轮排序，最大的泡泡就会排到最后面，依次类推第二轮，第三轮。。。
 * *
 * @author gx*/
